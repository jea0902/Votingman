-- =============================================
-- 정산 완료시 알림 자동 생성 트리거
-- payout_history INSERT → notifications INSERT
-- =============================================

CREATE OR REPLACE FUNCTION create_payout_notification()
RETURNS TRIGGER AS $$
DECLARE
    poll_info RECORD;
    is_win BOOLEAN;
    is_draw BOOLEAN;
    profit NUMERIC;
    notification_title TEXT;
    notification_content TEXT;
    payout_display TEXT;
BEGIN
    -- 투표 정보 조회 (제목 생성용)
    SELECT 
        sp.poll_date,
        sp.market
    INTO poll_info
    FROM sentiment_polls sp 
    WHERE sp.id = NEW.poll_id;
    
    -- 수익/손실 계산
    profit := NEW.payout_amount - NEW.bet_amount;
    is_win := profit > 0;
    is_draw := profit = 0;
    
    -- 투표 제목 생성 (시장별 커스터마이징)
    CASE 
        WHEN poll_info.market = 'btc' THEN
            notification_title := '📊 비트코인 내일 상승/하락 예측';
        WHEN poll_info.market = 'ndq' THEN  
            notification_title := '📊 나스닥 내일 상승/하락 예측';
        WHEN poll_info.market = 'sp500' THEN
            notification_title := '📊 S&P500 내일 상승/하락 예측';
        WHEN poll_info.market = 'kospi' THEN
            notification_title := '📊 코스피 내일 상승/하락 예측';
        WHEN poll_info.market = 'kosdaq' THEN
            notification_title := '📊 코스닥 내일 상승/하락 예측';
        ELSE
            notification_title := '📊 ' || COALESCE(poll_info.market, '시장') || ' 투표 예측';
    END CASE;
    
    -- 정산 결과 표시 텍스트 생성
    IF is_draw THEN
        payout_display := '원금 반환 (' || NEW.payout_amount || ' VTC)';
    ELSIF is_win THEN
        payout_display := '+' || profit || ' VTC';
    ELSE
        payout_display := profit || ' VTC';  -- 음수이므로 - 기호 자동 포함
    END IF;
    
    -- 알림 내용 생성
    notification_content := '정산이 완료되었습니다. ' || payout_display;
    
    -- 알림 레코드 삽입
    INSERT INTO notifications (
        user_id, 
        type, 
        title, 
        content, 
        metadata, 
        related_payout_id
    ) VALUES (
        NEW.user_id,
        'payout',
        notification_title,
        notification_content,
        jsonb_build_object(
            'payout_id', NEW.id,
            'poll_id', NEW.poll_id,
            'poll_date', poll_info.poll_date,
            'market', poll_info.market,
            'bet_amount', NEW.bet_amount,
            'payout_amount', NEW.payout_amount,
            'profit', profit,
            'is_win', is_win,
            'is_draw', is_draw,
            'redirect_url', '/profile/stats'
        ),
        NEW.id
    );
    
    -- 로그 출력 (개발용)
    RAISE LOG 'Payout notification created for user % (poll: %, profit: %)', 
        NEW.user_id, NEW.poll_id, profit;
    
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

-- 기존 트리거가 있다면 삭제 후 재생성
DROP TRIGGER IF EXISTS payout_notification_trigger ON payout_history;

CREATE TRIGGER payout_notification_trigger
    AFTER INSERT ON payout_history
    FOR EACH ROW
    EXECUTE FUNCTION create_payout_notification();

-- 트리거 생성 확인
-- INSERT INTO payout_history (poll_id, user_id, market, bet_amount, payout_amount)
-- VALUES ('test-poll-id', 'test-user-id', 'btc', 500, 750);
-- SELECT * FROM notifications WHERE user_id = 'test-user-id';