...
export async function GET(request: Request) {
  ...
  try {
    const rows = await fetchKlinesKstAligned("xrp_15m", 1);
    ...
    if (rows.length > 0) {
      ...
      settle = await settlePoll("", "xrp_15m", candleStartAtIso);
      if (
        settle.status === "settled" ||
        settle.status === "invalid_refund"
      ) {
        refreshMarketStats(TIER_MARKET_ALL).catch((refreshErr) => {
          console.error("[cron/xrp-ohlc-15m] refreshMarketStats 실패 (정산은 완료됨):", refreshErr);
        });
      }
    } else {
      const fallbackStartAts = getRecentCandleStartAts("xrp_15m", 1);
      if (fallbackStartAts[0]) {
        settle = await settlePoll("", "xrp_15m", toCanonicalCandleStartAt(fallbackStartAts[0]));
        if (
          settle.status === "settled" ||
          settle.status === "invalid_refund"
        ) {
    ...
    return NextResponse.json({
      success: true,
      data: {
        message: rows.length > 0 ? "xrp_15m 수집 및 정산 완료" : "xrp_15m 캔들 미수집(Binance 빈 응답), 정산만 시도 완료",
      ...
      message.includes("Binance") || message.includes("klines")
        ...
        : message.includes("btc_ohlc") || message.includes("upsert")
          ...
          : message.includes("환불") || message.includes("정산") || message.includes("users")
            ? "SETTLEMENT_ERROR"
            : "CRON_ERROR";
    console.error("[cron/xrp-ohlc-15m] error:", code, e);
    const context: Record<string, unknown> = { market: "xrp_15m" };
    try {
      const startAts = getRecentCandleStartAts("xrp_15m", 1);
      if (startAts[0]) context.candle_start_at = startAts[0];
    } catch (_) {}
    try {
      await recordCronError("xrp-ohlc-15m", code, message, context);
    } catch (_) {}
    return NextResponse.json(
