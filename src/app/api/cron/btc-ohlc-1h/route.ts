      ...
      message.includes("Binance") || message.includes("klines")
        ...
        : message.includes("btc_ohlc") || message.includes("upsert")
          ...
          : message.includes("환불") || message.includes("정산") || message.includes("users")
            ? "SETTLEMENT_ERROR"
            : "CRON_ERROR";
    console.error("[cron/btc-ohlc-1h] error:", code, e);
    const context: Record<string, unknown> = { market: "btc_1h" };
    try {
      const startAts = getRecentCandleStartAts("btc_1h", 1);
      if (startAts[0]) context.candle_start_at = startAts[0];
    } catch (_) {}
    try {
      await recordCronError("btc-ohlc-1h", code, message, context);
    } catch (_) {}
    return NextResponse.json(
