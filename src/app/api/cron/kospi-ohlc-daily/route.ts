/**
 * 매 영업일 KST 15:30 직후에 실행: kospi_1d 당일 봉 수집 → korea_ohlc 저장
 * - candle_start_at: 해당 거래일 UTC 00:00 기준
 * - Yahoo ^KS11 일봉 수집 (당일 일봉, 09:00~15:30 장 전체를 포괄)
 *
 * URL: GET https://<도메인>/api/cron/kospi-ohlc-daily
  ...
  try {
    ...
    if (rows.length === 0) {
      return NextResponse.json({
        success: true,
        data: {
          message: "kospi_1d 캔들 미수집(Yahoo 빈 응답)",
          total_fetched: 0,
          upserted: 0,
          errors: 0,
        },
    ...
    const code = message.includes("Yahoo")
      ...
      : message.includes("korea_ohlc") || message.includes("upsert")
        ? "DB_UPSERT_ERROR"
        : "CRON_ERROR";
    console.error("[cron/kospi-ohlc-daily] error:", code, e);
    ...
    try {
      await recordCronError("kospi-ohlc-daily", code, message, context);
    } catch (_) {}
    return NextResponse.json(
