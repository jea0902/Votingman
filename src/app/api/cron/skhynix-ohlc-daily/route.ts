/**
 * 매 영업일 KST 15:30 직후에 실행: skhynix_1d 당일 봉 수집 → korea_ohlc 저장
 * - candle_start_at: 해당 거래일 UTC 00:00 기준
 * - Yahoo 000660.KS(SK 하이닉스) 일봉 수집
 *
 * URL: GET https://<도메인>/api/cron/skhynix-ohlc-daily
  ...
  try {
    ...
    if (rows.length === 0) {
      return NextResponse.json({
        success: true,
        data: {
          message: "skhynix_1d 캔들 미수집(Yahoo 빈 응답)",
          total_fetched: 0,
          upserted: 0,
          errors: 0,
        },
    ...
    const code = message.includes("Yahoo")
      ...
      : message.includes("korea_ohlc") || message.includes("upsert")
        ? "DB_UPSERT_ERROR"
        : "CRON_ERROR";
    console.error("[cron/skhynix-ohlc-daily] error:", code, e);
    ...
    try {
      await recordCronError("skhynix-ohlc-daily", code, message, context);
    } catch (_) {}
    return NextResponse.json(
