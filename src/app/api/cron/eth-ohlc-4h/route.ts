...
export async function GET(request: Request) {
  ...
  try {
    const rows = await fetchKlinesKstAligned("eth_4h", 1);
    ...
    if (rows.length > 0) {
      ...
      settle = await settlePoll("", "eth_4h", candleStartAtIso);
      if (settle.status === "already_settled" && settle.error?.includes("폴을 찾을 수 없습니다")) {
        console.warn("[cron/eth-ohlc-4h] 정산 대상 폴 없음", {
          candle_start_at: candleStartAtIso,
          hint: "sentiment_polls에 (market=eth_4h, candle_start_at) 일치하는 폴이 있는지 확인. 미정산 폴은 POST /api/admin/backfill-and-settle 에 pollIds 로 요청.",
        });
      }
      if (
        ...
        refreshMarketStats(TIER_MARKET_ALL).catch((refreshErr) => {
          console.error("[cron/eth-ohlc-4h] refreshMarketStats 실패 (정산은 완료됨):", refreshErr);
        });
      }
    }
      ...
      message.includes("Binance") || message.includes("klines")
        ...
        : message.includes("btc_ohlc") || message.includes("upsert")
          ...
          : message.includes("환불") || message.includes("정산") || message.includes("users")
            ? "SETTLEMENT_ERROR"
            : "CRON_ERROR";
    console.error("[cron/eth-ohlc-4h] error:", code, e);
    const context: Record<string, unknown> = { market: "eth_4h" };
    try {
      const startAts = getRecentCandleStartAts("eth_4h", 1);
      if (startAts[0]) context.candle_start_at = startAts[0];
    } catch (_) {}
    try {
      await recordCronError("eth-ohlc-4h", code, message, context);
    } catch (_) {}
    return NextResponse.json(
      {
        success: false,
