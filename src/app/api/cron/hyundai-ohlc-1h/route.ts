/**
 * 매 영업일 KST 10:00~15:00 정각에 실행: hyundai_1h 직전 1시간 봉 수집 → korea_ohlc 저장
 * - 예) 10:00 실행 시 09:00~10:00 KST 1시간봉, 15:00 실행 시 14:00~15:00 KST 1시간봉
 * - Yahoo 005380.KS(현대자동차) 1시간봉 수집
 *
 * URL: GET https://<도메인>/api/cron/hyundai-ohlc-1h
  ...
  try {
    ...
    if (rows.length === 0) {
      return NextResponse.json({
        success: true,
        data: {
          message: "hyundai_1h 캔들 미수집(Yahoo 빈 응답)",
          total_fetched: 0,
          upserted: 0,
          errors: 0,
        },
    ...
    const code = message.includes("Yahoo")
      ...
      : message.includes("korea_ohlc") || message.includes("upsert")
        ? "DB_UPSERT_ERROR"
        : "CRON_ERROR";
    console.error("[cron/hyundai-ohlc-1h] error:", code, e);
    ...
    try {
      await recordCronError("hyundai-ohlc-1h", code, message, context);
    } catch (_) {}
    return NextResponse.json(
