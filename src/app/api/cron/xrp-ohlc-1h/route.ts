/**
 * 매시 정각 실행: xrp_1h 최근 마감 캔들 수집 + 해당 봉 정산
 * - 실제: cron-job.org에서 정시 호출 (평균 13초 지연)
 *
 * GET /api/cron/xrp-ohlc-1h
...
export async function GET(request: Request) {
  ...
  try {
    const rows = await fetchKlinesKstAligned("xrp_1h", 1);
    ...
    if (rows.length > 0) {
      ...
      settle = await settlePoll("", "xrp_1h", candleStartAtIso);
      if (
        settle.status === "settled" ||
        settle.status === "invalid_refund"
      ) {
        refreshMarketStats(TIER_MARKET_ALL).catch((refreshErr) => {
          console.error("[cron/xrp-ohlc-1h] refreshMarketStats 실패 (정산은 완료됨):", refreshErr);
        });
      }
    }
      ...
      message.includes("Binance") || message.includes("klines")
        ...
        : message.includes("btc_ohlc") || message.includes("upsert")
          ...
          : message.includes("환불") || message.includes("정산") || message.includes("users")
            ? "SETTLEMENT_ERROR"
            : "CRON_ERROR";
    console.error("[cron/xrp-ohlc-1h] error:", code, e);
    const context: Record<string, unknown> = { market: "xrp_1h" };
    try {
      const startAts = getRecentCandleStartAts("xrp_1h", 1);
      if (startAts[0]) context.candle_start_at = startAts[0];
    } catch (_) {}
    try {
      await recordCronError("xrp-ohlc-1h", code, message, context);
    } catch (_) {}
    return NextResponse.json(
