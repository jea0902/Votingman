/**
 * 매일 KST 09:00에 실행: xrp_1d 최근 마감 캔들 수집 + 해당 봉 정산
 *
 * btc-ohlc-daily와 동일한 로직: fetchKlinesKstAligned → upsertBtcOhlcBatch → settlePoll
...
export async function GET(request: Request) {
  ...
  try {
    let settle = null;
    let rows = await fetchKlinesKstAligned("xrp_1d", 1);
    if (rows.length === 0) {
      const fallbackStartAts = getRecentCandleStartAts("xrp_1d", 1);
      if (fallbackStartAts[0]) {
        settle = await settlePoll("", "xrp_1d", fallbackStartAts[0]);
        if (
          settle.status === "settled" ||
          settle.status === "invalid_refund"
        ) {
          refreshMarketStats(TIER_MARKET_ALL).catch((refreshErr) => {
            console.error("[cron/xrp-ohlc-daily] refreshMarketStats 실패:", refreshErr);
          });
        }
        return NextResponse.json({
          success: true,
          data: {
            message: "xrp_1d 캔들 미수집(Binance 빈 응답), 정산만 시도 완료",
            total_fetched: 0,
            upserted: 0,
            errors: 0,
            settle,
    ...
    if (rows.length > 0) {
      ...
      settle = await settlePoll("", "xrp_1d", toCanonicalCandleStartAt(justClosed.candle_start_at));
      if (
        settle.status === "settled" ||
        settle.status === "invalid_refund"
      ) {
        refreshMarketStats(TIER_MARKET_ALL).catch((refreshErr) => {
          console.error("[cron/xrp-ohlc-daily] refreshMarketStats 실패 (정산은 완료됨):", refreshErr);
        });
      }
    }
      ...
      message.includes("Binance") || message.includes("klines")
        ...
        : message.includes("btc_ohlc") || message.includes("upsert")
          ...
          : message.includes("환불") || message.includes("정산") || message.includes("users")
            ? "SETTLEMENT_ERROR"
            : "CRON_ERROR";
    console.error("[cron/xrp-ohlc-daily] error:", code, e);
    const context: Record<string, unknown> = { market: "xrp_1d" };
    try {
      const startAts = getRecentCandleStartAts("xrp_1d", 1);
      if (startAts[0]) context.candle_start_at = startAts[0];
    } catch (_) {}
    try {
      await recordCronError("xrp-ohlc-daily", code, message, context);
    } catch (_) {}
    return NextResponse.json(
