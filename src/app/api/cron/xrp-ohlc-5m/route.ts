...
export async function GET(request: Request) {
  ...
  try {
    const rows = await fetchKlinesKstAligned("xrp_5m", 1);
    ...
    if (rows.length > 0) {
      ...
      settle = await settlePoll("", "xrp_5m", candleStartAtIso);
      if (settle.error) {
      ...
      if (
        ...
        refreshMarketStats(TIER_MARKET_ALL).catch((refreshErr) => {
          console.error("[cron/xrp-ohlc-5m] refreshMarketStats 실패 (정산은 완료됨):", refreshErr);
        });
      }
    }
      ...
      message.includes("Binance") || message.includes("klines")
        ...
        : message.includes("btc_ohlc") || message.includes("upsert")
          ...
          : message.includes("환불") || message.includes("정산") || message.includes("users")
            ? "SETTLEMENT_ERROR"
            : "CRON_ERROR";
    console.error("[cron/xrp-ohlc-5m] error:", code, e);
    const context: Record<string, unknown> = { market: "xrp_5m" };
    try {
      const startAts = getRecentCandleStartAts("xrp_5m", 1);
      if (startAts[0]) context.candle_start_at = startAts[0];
    } catch (_) {}
    try {
      await recordCronError("xrp-ohlc-5m", code, message, context);
    } catch (_) {}
    return NextResponse.json(
